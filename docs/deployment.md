...
This repository deploys the Next.js H5 site as a static website to Baidu Cloud BOS. GitHub Actions builds the static export, verifies the exported files, installs the Baidu BOS CLI, and runs the same deployment script that can be used locally.
...
GitHub push to main/master or manual workflow_dispatch
  ...
  -> ESLint
  -> Next.js static export into out/
  -> export verification
...
4. Run `npm run lint`.
5. Run `npm run build`, which uses Next.js static export.
6. Verify `out/index.html`, `out/_next/`, and the absence of server-only `BUILD_ID` output.
7. Install `bcecmd`.
8. Run `scripts/deploy.sh`.

## Static export requirements

`next.config.ts` must keep these settings for BOS static hosting:
...
| Variable | Purpose |
| --- | --- |
| `OUT_DIR` | Override the export directory. Defaults to `out`. |
...
Check that `next.config.ts` still uses static export settings and that `npm run build` produces `out/index.html` and `out/_next/`.
...
Verify the deploy logs show `_next` assets being uploaded and that `out/_next/` exists after `npm run build`.
