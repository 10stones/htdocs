...
| `BOS_REGION` | Optional BOS bucket region. Defaults to `bj`.           |

The workflow reads optional repository variable `BOS_REGION` and falls back to `bj`. Override `BOS_ENDPOINT` only when the default `https://<region>.bcebos.com` endpoint is not suitable.
...
If `scripts/upload_bos.py` reports a missing environment variable, verify the corresponding GitHub Secret or repository variable exists and is spelled exactly as documented.
